...
// Client Component - Solo para interactividad (botón volver, navegación)
// Los datos se reciben como props desde el Server Component padre (SSR)
...
export default function MovieDetailsClient({ movie, city }) {
        ...
        {movie.showtimes?.length > 0 ? (
          <div className="flex flex-wrap gap-3">
            {movie.showtimes.map((time, index) => (
              <Link
                key={index}
                href={`/movie/${movie.id}/session/${time}?city=${city}`}
                prefetch={false}
          ...
          <p className="text-cinema-text-muted">No hay sesiones disponibles en {cityName}</p>
        )}
      </section>
    </div>
  );
