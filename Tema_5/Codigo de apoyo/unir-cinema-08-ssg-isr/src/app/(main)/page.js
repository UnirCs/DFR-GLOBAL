...
import { getCinemas, getMetrics, getTopMovies } from '@/lib/api';
...
export default async function HomePage() {
  // Obtener datos - diferentes estrategias de cache:
  const cinemas = await getCinemas();       // force-cache (permanente)
  const metrics = await getMetrics();        // Se regenera con ISR de la página (15s)
  const topMovies = await getTopMovies();    // Cache de datos propio (30s)
