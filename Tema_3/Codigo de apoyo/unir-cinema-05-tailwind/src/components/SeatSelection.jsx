...
const SeatSelection = ({ movieId, time }) => {
  ...
  const getSeatClasses = (seat) => {
    const base = "aspect-square rounded-lg flex items-center justify-center font-bold text-xs transition-all duration-300 border-2";
    if (seat.isOccupied) {
      return `${base} bg-cinema-red/70 text-white border-cinema-red-dark cursor-not-allowed`;
    }
    if (selectedSeats.includes(seat.id)) {
      return `${base} bg-cinema-gold text-cinema-dark border-cinema-gold shadow-lg shadow-cinema-gold/50 cursor-pointer`;
    }
    return `${base} bg-green-600 text-white border-green-600 cursor-pointer hover:scale-110 hover:shadow-lg hover:shadow-green-500/50`;
  };
    ...
    return (
      <div className="flex-1 p-4 md:p-8 max-w-7xl mx-auto w-full">
        <div className="bg-gradient-to-br from-cinema-dark-card to-cinema-dark-elevated p-8 rounded-2xl shadow-lg border border-cinema-border text-center">
          ...
          <Link href="/" className="inline-block bg-gradient-to-r from-cinema-gold to-cinema-gold-dark text-cinema-dark px-6 py-3 rounded-lg font-bold hover:brightness-110 transition-all">
            Volver al inicio
          </Link>
        </div>
      </div>
