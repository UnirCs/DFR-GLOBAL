...
import { getCinemasFromStore, getMetricsFromStore, getTopMoviesFromStore } from '@/lib/api';
...
export default async function HomePage() {
  // Obtener datos directamente del store (sin fetch)
  // Esto funciona tanto en build time como en runtime
  const cinemas = getCinemasFromStore();     // Lista de cines
  const metrics = getMetricsFromStore();     // Métricas del sistema
  const topMovies = getTopMoviesFromStore(); // Películas ordenadas por rating
            ...
            {topMovies.map((movie, index) => (
              ...
              >
                ...
                <div className="relative h-48 overflow-hidden">
                  <Image
                    src={movie.poster || '/film-poster.jpg'}
                    alt={movie.title}
                    fill
