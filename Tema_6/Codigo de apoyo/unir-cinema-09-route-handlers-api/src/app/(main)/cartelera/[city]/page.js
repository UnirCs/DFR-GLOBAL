...
import { getCinemasFromStore, getCinemaMoviesWithDetailsFromStore } from '@/lib/api';
...
export default async function CarteleraPage({ params }) {
  ...
  // Validar que la ciudad existe (acceso directo al store)
  const cinemas = getCinemasFromStore();
  ...
  // Obtener películas con sus detalles (acceso directo al store)
  const movies = getCinemaMoviesWithDetailsFromStore(city);
